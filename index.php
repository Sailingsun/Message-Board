...
<center>
    ...
    <?PHP
		session_start();
		if($_SESSION){    //清除登录痕迹 防止下次直接登录
			session_unset();
			session_destroy();
			}
	?>
    未登录,请您登录
<button onclick="javascript:window.location.href='login.html'" align="right" style="font-size:18px; width:80px; height:40px;">登录</button>
...
<tr><td colspan="2" align="right"><input type="submit" value="提交留言" onclick="return ifLogin()"/>    <input type="reset" value="重置"></td></tr>
    </table>
    </form>
    <hr>
...
<input type="submit" value="搜索" />    <input type="button" onclick="javascript:window.location='index.php'" value="全部"><br/><br/><br/>
    </form>
 <!------------------------->   
...
<?PHP
	...
	while($row = mysql_fetch_assoc($result)){
		...
		echo "<td><a href='action.php?action=del&id={$row['id']}&username={$row['username']}'>删除</a> 
